...
template<int dim>
int HeatEquation<dim>::get_refine_times()
{
	return refine_times;
}

template<int dim>
void HeatEquation<dim>::get_x(std::vector<double> & in)
{
	in.resize(N);
...
template<int dim>
void HeatEquation<dim>::set_refine_times(int in)
{
	refine_times = in;
}

template<int dim>
void HeatEquation<dim>::set_N(int in)
{
	N = in;
}

template<int dim>
int HeatEquation<dim>::get_N() const
{
	return N;
}
template<int dim>
void HeatEquation<dim>::set_local_iteration(int in)
{
	local_iteration = in;
}

template<int dim>
int HeatEquation<dim>::get_local_iteration() const
{
	return local_iteration;
}

template<int dim>
void HeatEquation<dim>::refine_mesh(std::vector<double> oldSolution,
...
#undef float
	...
	KellyErrorEstimator<dim>::estimate(dof_handler,
			QGauss<dim - 1>(fe.degree + 1), typename FunctionMap<dim>::type(),
			yita_full_2D_t, estimated_error_per_cell);
#define float double

//      GridRefinement::refine_and_coarsen_fixed_fraction (
//	  triangulation, estimated_error_per_cell, 1., 0.0);
	GridRefinement::refine(triangulation, estimated_error_per_cell, 0.50, 200);
	...
	spline_chen(&x[1], &oldSolution[1], &xp[1], &newSolution[1], x.size() - 2,
			xp.size() - 2, NULL);
...
template<int dim>
void HeatEquation<dim>::update_internal_data()
{
	time = 0.;
	timestep_number = 0;
	printf("N=%d  \n", N);
...
template<int dim>
void HeatEquation<dim>::get_f0_given()
{
	f0_given.reinit(N);
	for (int i = 0; i < N; i++)
		f0_given[i] = 1.;
...
template<int dim>
void HeatEquation<dim>::output_results_for_yita_full_2D() const
{
...
template<int dim>
void HeatEquation<dim>::output_mesh()
{
	const std::string filename = "yita_full_2D_N="
...
template<int dim>
void HeatEquation<dim>::print_and_save_yita_1D(std::vector<double> solution)
{
	printf("Solved ! \n middle_solution: N=%d \n", get_N());
...
template<int dim>
double HeatEquation<dim>::get_L()
{
	return L;
}

template<int dim>
double HeatEquation<dim>::get_mean_field_free_energy()
{
	double mean_field_free_energy = 0.;
...
//			printf("fi=%2.15f when x = %2.15f ,p[0]=%2.15f\n", fi, x, p[0]);
			mean_field_free_energy += old_solution_values[q_point] * fi
...
//	printf("f0_given_bar=%2.15f \n", f0_given_bar);
	mean_field_free_energy=(mean_field_free_energy/f0_given_bar/L+log(f0_given_bar))/(-N);
	return mean_field_free_energy;
}

template<int dim>
void HeatEquation<dim>::set_yita_full_2D()
{
	std::vector<double> x;
	get_x(x);
	...
	double m = 0.;
	spline_chen(&x[0], yita_middle_1D, xp, yp, x.size(), support_points.size(),
			&m);
...
template<int dim>
void HeatEquation<dim>::build_lookup_table()
{
	std::vector<double> x;
	get_x(x);
...
template<int dim>
void HeatEquation<dim>::assemble_system() /* assemble for A,B,C
 ...
 /*/
{
	...
	VectorTools::interpolate_boundary_values(dof_handler, 0,
			ZeroFunction<dim>(), constraint_matrix);
...
//	qpositions=fe_values.get_quadrature_points();
			...
			for (unsigned int i = 0; i < dofs_per_cell; ++i)
				for (unsigned int j = 0; j < dofs_per_cell; ++j)
				{
					...
					cell_C(i, j) += fe_values.shape_value(i, q_point)
							* fe_values.shape_value(j, q_point)
							* old_solution_values[q_point]
							* fe_values.JxW(q_point);
				}

		cell->get_dof_indices(local_dof_indices);

		constraint_matrix.distribute_local_to_global(cell_A, local_dof_indices,
				A);
		constraint_matrix.distribute_local_to_global(cell_B, local_dof_indices,
				B);
		constraint_matrix.distribute_local_to_global(cell_C, local_dof_indices,
				C);
	}
	...
	for (unsigned int i = 0; i < A.m(); i++)
	{
		...
		for (; begin != end; ++begin)
		{
			...
			// block(1,1)
			systemMatrixBlock.set(row + n_dof, col + n_dof,
					A(row, col) + time_step / 4 * D(row, col));
		}
	}
