...
double* adm(double *x, int n, int *check,
		void (*funcvmix)(int, double *x, double *xnew), int flag) {
	double err = 1e-10;
...
//			debug=100;
			...
			//for (i = 0; i < n; i++)
			//printf("%d,xnew= %22.14e, x= %22.14e error=%22.14e  \n",i, xnew[i], x[i],xnew[i]-x[i]);
			printf("Solve sucessfully !!!!!!!!!!!! error =%21.14e,TOLF=%21.14e!!!!!!!!!!!! \n",err,TOLF);
		...
		if (its >= ERRMAX * 3) {
			i = its % ERRMAX;
			if (errhist[i] > errhist[i == ERRMAX - 1 ? 0 : i + 1]
					|| err >= 1E-4) {
				*check = 1;
				printf("solve failed  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n");
				FREE_RETURN
